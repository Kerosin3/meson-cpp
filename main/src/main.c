...
signed long long read_bytes(FILE*, unsigned, unsigned,
			    ...);    // retuns last byte number
unsigned long find_valid_jpg(FILE*); // __attribute__ ((nwarn_wint_conversion));
unsigned long
find_zip_filename(FILE*); //__attribute__ ((Nwarn_wint_conversion));
...
int main(int argc, char* argv[])
{
	...
	if (!(jpg_end = find_valid_jpg(
		      fp))) { // find whether this is a valid jpg file
		printf("jpg signatures has not been found, you passed not a valid jpg file, exiting...\n");
		exit(1);
	}
	while ((first_signature_zip = find_zip_filename(
			fp))) { // repead find zip header signature and extract
				// a filename till EOF
#ifdef DEBUG
...
#endif
		extract_filename(fp,
				 first_signature_zip); // extract filename and
						       // add to a global offset
		n_files++;
	} // while not EOF
	global_offset = 0x0;
	fclose(fp);
...
signed long extract_filename(FILE* fp, unsigned long offset)
{
...
#endif
	const unsigned filename_length =
		2; // for length, not for filename itself
	...
	fread(&readed_length, 1, filename_length,
	      fp); // read 2 bytes for filename
#ifdef DEBUG
	...
	printf("\n");
	global_offset =
		offset + 0xF + 0x7; // set global adress to extra fiel offse
...
unsigned long find_zip_filename(FILE* fp)
{
	...
	signarute_offset = read_bytes(fp, global_offset, 4, header_start[3],
				      header_start[2], header_start[1],
				      header_start[0]); //  REVERSE OREDER
	if (signarute_offset >= 0) {
		...
		return (unsigned long)
			signarute_offset; // return first signature offset
	} else {
#ifdef DEBUG
...
unsigned long find_valid_jpg(FILE* fp)
{
	...
	if ((out = read_bytes(fp, global_offset, 2, jpg_start[1], jpg_start[0]))
	    > 0) {
	...
	if ((out = read_bytes(fp, global_offset, 2, jpg_end[1], jpg_end[0]))
	    > 0) {
...
signed long long read_bytes(FILE* fp, unsigned skip, unsigned n_width, ...)
{
	...
	if (n_calls == 2)
		printf("Not valid zip signature found in the file, exiting...\n"); // detect not valid zip jpg
	else
		printf("EOF reached \n");
	va_end(ap);
	return -2;
